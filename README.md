...
- 📋 **粘贴自动填充**：无需导入账号，直接从 Excel 选中「信用代码、用户名、密码」三个单元格复制，在登录页按 `Ctrl+V`（Mac 为 `Cmd+V`）即可一键填入。
...
适合临时登录一家还没导入的公司，或换台电脑不想重导：

1. 在 Excel 里选中某一行的「信用代码、用户名、密码」**三个单元格**，`Ctrl+C` 复制。
   - 横排一排（同一行相邻三列）或竖排一列（同一列相邻三行）都行，Excel 复制到剪贴板后都是空白分隔的三段。
2. 切到税局登录页，在页面**任意位置**按 `Ctrl+V`（Mac 为 `Cmd+V`）。
3. 插件识别到剪贴板恰好是三段，自动按「信用代码 → 用户名 → 密码」填入对应登录框，并聚焦密码框方便直接登录。

> 仅当剪贴板内容 trim 后**恰好 3 段**时才触发；复制其它内容（如 4 段、长文本）不会影响你在页面里的正常粘贴。
...
税局新版登录页（`tpass.guangdong.chinatax.gov.cn`）基于 Vue + ElementUI。**搜索填充和粘贴填充都走同一套定位逻辑**，按**优先级从高到低**：
